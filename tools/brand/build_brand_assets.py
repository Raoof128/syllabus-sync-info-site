...
DERIVATIVES = (
    ...
    # Compact copy of the lockup for the Open Graph card, which inlines the file
    # as a data URI at render time and so must stay small.
    Derivative("Syllabus_Sync_Logo_Primary_Horizontal.png", "og-lockup.png", 0.04, 520),
)
...
def main() -> None:
    ...
    # App Router conventions: these filenames are picked up automatically and
    # emitted as <link rel="icon"> / <link rel="apple-touch-icon">.
    routed = {
        APP / "icon.png": build_icon(mark, 512, 0.80),
        APP / "apple-icon.png": build_icon(mark, 180, 0.74),
    }
    print("app router icons:")
    for dest, icon in routed.items():
        icon.save(dest, format="PNG", optimize=True)
        print(f"  {dest.name:<32} {icon.width}x{icon.height}  {dest.stat().st_size / 1024:.0f} KB")
